...
if [ "$1" = "dev" ]; then
  ...
  webVersion=$(wget -qO- -t1 -T2 "https://api.github.com/repos/friedHDD/alist-web/releases/latest" | grep "tag_name" | head -n 1 | awk -F ":" '{print $2}' | sed 's/\"//g;s/,//g;s/ //g')
fi
...
ldflags="\
-w -s \
-X 'github.com/NodeSeekDev/alist/v3/internal/conf.BuiltAt=$builtAt' \
-X 'github.com/NodeSeekDev/alist/v3/internal/conf.GitAuthor=$gitAuthor' \
-X 'github.com/NodeSeekDev/alist/v3/internal/conf.GitCommit=$gitCommit' \
-X 'github.com/NodeSeekDev/alist/v3/internal/conf.Version=$version' \
-X 'github.com/NodeSeekDev/alist/v3/internal/conf.WebVersion=$webVersion' \
"

FetchWebDev() {
  curl -L https://github.com/friedHDD/alist-web/releases/download/beta/dist.tar.gz -o dist-beta.tar.gz
  tar -zxvf dist-beta.tar.gz
  rm -rf public/dist
  mv -f dist public
  rm -rf dist dist-beta.tar.gz
}

FetchWebRelease() {
  curl -L https://github.com/friedHDD/alist-web/releases/latest/download/dist.tar.gz -o dist.tar.gz
